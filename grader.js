...
var checkHtmlFile = function(htmlfile, checksfile) {
    var html = loadHtmlFile(htmlfile);
    runChecks(html,checksfile); 
};
...
var checkHtmlUrl = function(htmlUrl, checksfile) {
  ...
  rest.get(htmlUrl).on('complete', function(result) {
    if (result instanceof Error) {
      console.log("GET of %s retuned error: %s. Exiting.", instr, result.message); 
      process.exit(1);
    } else {
      //console.log(result);
...
if(require.main == module) {
    program
        ...
        .option('-u, --url <url>', 'url of html to be graded')
        .parse(process.argv);
